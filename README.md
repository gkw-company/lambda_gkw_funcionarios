...
```bash
lambda-api-funcionarios$ sam build --use-container
```
...
```bash
lambda-api-funcionarios$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
lambda-api-funcionarios$ sam local start-api
lambda-api-funcionarios$ curl http://localhost:3000/
```
...
```bash
lambda-api-funcionarios$ sam logs -n HelloWorldFunction --stack-name "lambda-api-funcionarios" --tail
```
...
```bash
lambda-api-funcionarios$ pip install -r tests/requirements.txt --user
# unit test
lambda-api-funcionarios$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
lambda-api-funcionarios$ AWS_SAM_STACK_NAME="lambda-api-funcionarios" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "lambda-api-funcionarios"
```
